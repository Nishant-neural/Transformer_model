...
class ScaledDotProductAttention:
    ...
    def forward(self , Q, K, V, mask=None):
        ...
        self.scores = Q @ np.swapaxes(K, -1, -2)
        self.scores = self.scores / np.sqrt(self.d_k)
        ...
        self.weights = softmax_forward(self.scores)
        self.out = self.weights @ V

     # weights = batch ,heads, seq_lenq , seq_len_k
        # Out = batch ,heads , seq_len_q , d_v
    ...
    def backward(self , d_out ):
        batch, head_num, seq_len_q, d_k =  self.Q.shape


        self.dV =  self.weights.transpose(0,1,3,2)  @ d_out

        self.dweights = d_out @ self.V.transpose(0,1,3,2)
       ...
       # scores = scores/ scaling factor
        ...
        self.dscores = self.dscaled / np.sqrt(d_k)
        self.dQ = self.dscores @ self.K
        self.dK = self.dscores.transpose(0,1,3,2) @ self.Q

        return self.dV , self.dQ , self.dK
